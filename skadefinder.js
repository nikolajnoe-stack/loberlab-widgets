// LøberLab Skadefinder Widget v1.2
...
window.llCloseModalOnOverlay = window.llCloseModalOnOverlay || function(e) {
  ...
  window.llBookSecondOpinion = function() {
    ...
    sendToSheets({
      event: 'second_opinion_requested', injury: top ? top.id : null,
      area: s.answers.area || '', timestamp: new Date().toISOString()
    });
    s.step = 8;
    render();
  };
  ...
  window.llSubmitEmail = function() {
    ...
    sendToSheets({
      type: s.wantsSecondOpinion ? 'second_opinion' : 'plan',
      name: nm, email: em,
      injury_name: top ? top.n : '', injury_id: top ? top.id : '',
      match_score: top ? top.matchScore : 0,
